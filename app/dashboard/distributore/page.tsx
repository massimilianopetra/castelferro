...
export default function DistributorePage() {
    ...
    const handleOpenStatoConti = async () => {
        // Se le statistiche sono disabilitate, blocca l'esecuzione senza interrogare il DB
    ...
    return (
        <ThemeProvider theme={defaultTheme}>
            <Box sx={{
                ...
                {/* MODIFICA: Nasconde il pulsante Stato conti se le statistiche sono disabilitate */}
                {!disabilitaStatistiche && (
                ...
                {/* MODIFICA: Il pulsante Stima attesa era già vincolato, rimane intatto */}
                {!disabilitaStatistiche && (
                ...
                <Box sx={{ width: '100%', maxWidth: '800px', mt: 1, pt: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Paper elevation={1} sx={{ p: 1, borderRadius: '15px', bgcolor: '#fff', position: 'relative' }}>
                        {/* MODIFICA: Inserimento dell'interruttore Switch per gestire lo stato "Off Statistiche" */}
                        <Box sx={{ position: 'absolute', top: 4, right: 12, zIndex: 5 }}>
                            <FormControlLabel
                                control={
                                    <Switch
                                ...
                                label={<Typography sx={{ fontWeight: 'bold', fontSize: '0.72rem' }}>Off Statistiche</Typography>}
                            />
                        </Box>

                        <Typography variant="caption" sx={{ ml: 1, fontWeight: 'bold', color: 'text.secondary' }}>MODALITÀ</Typography>
                        <Stack direction="row" spacing={1} sx={{ mt: 0, pr: { xs: 15, sm: 16 } }}>
